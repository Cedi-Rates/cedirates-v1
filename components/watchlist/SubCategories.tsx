...
const SubCategory = ({
  ...
  useEffect(() => {
    ...
    if (pathname.includes("usd-to-ghs")) {
      path = pathname.replace("/watchlist/usd-to-ghs", "");
    } else if (pathname.includes("eur-to-ghs")) {
      path = pathname.replace("/watchlist/eur-to-ghs", "");
    } else if (pathname.includes("gbp-to-ghs")) {
      path = pathname.replace("/watchlist/gbp-to-ghs", "");
    } else path = pathname;
  ...
  const handleSubcategoryClick = (subcategory: (typeof subCategories)[0]) => {
    ...
    if (subcategory.name === "All") {
      ...
      newUrl = currency
        ? `/watchlist/${currency}-to-ghs${subcategory.link}`
        : `/watchlist/usd-to-ghs${subcategory.link}`;
    }
        ...
        <div className="flex">
          {
            <Button
              ...
              className={
                pathname === "/watchlist/" ||
                pathname === "/watchlist/usd-to-ghs/" ||
                pathname === "/watchlist/eur-to-ghs/" ||
                pathname === "/watchlist/gbp-to-ghs/"
