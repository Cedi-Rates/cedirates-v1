...
const HomeDialog = ({
  ...
  const [openHomeRegion, setOpenHomeRegion] = useState(false);
  const [openHomeTown, setOpenHomeTown] = useState(false);

  const user = useContext(UserContext);
  const watchTown = watch("homeLocation.town");
  const watchRegion = watch("homeLocation.region");
  ...
  }, [user, user?.homeLocation?.region, user?.homeLocation?.town]);
  ...
  return (
    <div className={style.homeDialog}>
      <Dialog open={open} onOpenChange={handleClose}>
        ...
        >
          ...
          <div className="scroll-smooth">
            {/* Home Region */}
            <Popover open={openHomeRegion} onOpenChange={setOpenHomeRegion}>
                      ...
                      {regions.map((region, index) => (
                        <CommandItem
                          ...
                          onSelect={() => {
                            ...
                            setOpenHomeRegion(false);
                            setValue("homeLocation.region", region.name);
                          }}
                        >
                          <Check
                            className={cn(
            ...
            {/* Home Town */}
            <Popover open={openHomeTown} onOpenChange={setOpenHomeTown}>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  role="combobox"
                  ...
                  defaultValue={watchTown}
                >
                  {selectedTown
                    ? towns.find((town) => town.name === selectedTown)?.name ??
                      "Select town..."
                    : "Select town..."}
                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
                      ...
                      {towns?.map((town, index) => (
                        <CommandItem
                          ...
                          onSelect={() => {
                            ...
                            setValue("homeLocation.town", town.name);
                          }}
                        >
                          <Check
                            className={cn(
          ...
          <DialogFooter>
            <Button
              ...
              disabled={isSubmitting || !(selectedRegion && selectedTown)}
            >
              {/* <DialogClose> */}
              {isSubmitting ? (
